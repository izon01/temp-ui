  ...
  return (
    <div className="max-w-[1200px] mx-auto px-4 md:px-6 py-6 space-y-6">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-[#e1e3e4] rounded-xl p-6 shadow-sm flex flex-col justify-between h-40 hover:scale-[1.02] transition-transform cursor-pointer">
          <div className="flex justify-between items-start">
            <span className="text-sm text-[#434653]">멘토링 현황</span>
            <span className="material-symbols-outlined text-[#003e37]">groups</span>
          </div>
          <div>
            <span className="text-4xl font-bold text-[#191c1d]" style={{ fontFamily: 'Be Vietnam Pro, sans-serif' }}>12건</span>
            <p className="text-sm text-[#434653]">진행 중인 세션</p>
          </div>
          <div className="flex -space-x-2">
            {['A', 'B', 'C'].map((l, i) => (
              <div key={i} className="w-8 h-8 rounded-full border-2 border-white bg-[#0047ab] text-white text-xs flex items-center justify-center font-bold">{l}</div>
            ))}
            <div className="w-8 h-8 rounded-full border-2 border-white bg-[#434653] text-white text-[10px] flex items-center justify-center font-bold">+10</div>
          </div>
        </div>
      </section>
